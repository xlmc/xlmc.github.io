// YouTube Max Quality - Shadowrocket SABR rewriter v23
...
(function () {
  const PREFIX = "[YT Max SR v23][SABR]";
  ...
  function loadTargets() {
    ...
    const now = Date.now();
    return list.filter(x => x && x.version === 23 && x.capturedAt && now - Number(x.capturedAt) <= TTL_MS && Array.isArray(x.formats) && x.formats.length && Array.isArray(x.allFormats));
  }
